...
export default function App() {
  ...
  if (!me) {
    if (isCloud) {
      ...
      // "must_set_password" is handled inside CloudLogin itself (it owns the
      // in-progress Cognito challenge object) — just render the same screen.
      return <CloudLogin />;
    }
  ...
  return (
    <div className={`app ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
        ...
        <div className="nav-item" style={{ cursor: "default" }} title={collapsed ? (isCloud ? "AWS" : "Local demo") : undefined}>
          {isCloud ? <Cloud size={16} /> : <HardDrive size={16} />} <span className="nav-label">{isCloud ? "AWS" : "Local demo"}</span>
        </div>
        {!isCloud && (
